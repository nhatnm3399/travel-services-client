...
const ImageIllustation= (props)=> {
    ...
    const f= (e)=> {
        Object.values(e.target.files).map(item=> setListImage(prev=> ([...prev, {img: item, imgPreview: URL.createObjectURL(item), key: item.lastModified}])))
    }
    ...
    return (
        <div className={"fskjakejakwjaklawwa"} style={{width: "100%",}}>
            ...
            <div className={"fjajkjsklajraeweaa"} style={{width: '100%', padding: 20, display: "flex", position: "relative", background: "#fff", height: 400, flexWrap: "wrap"}}>
                {
                    isChooseImage=== true && listImage.map((item, key)=>  <div key={key} className={"dlakjklajwaasas"} style={{width: 150, height: 150, padding: 10, position: "relative"}}>
                        <img src={item.imgPreview} alt="open" style={{width: '100%', height: "100%", objectFit: "cover", border: "1px solid #e7e7e7"}} />
                        <div title={'Xóa'}  style={{display: 'flex', justifyContent: 'center', alignItems: 'center', position: "absolute", right: 0, top: 0}} onClick={()=> setListImage(listImage.filter(img=> parseInt(img.key) !== parseInt(item.key)))}>
                ...
                {
                    isChooseImage=== false &&
                    <div className={"fkdjksjakwjawawas"} style={{width: 80, height: 80, display: "flex", justifyContent: 'center', alignItems: "center", flexDirection: "column", position: "absolute", background: "#d9d9d9", cursor: "pointer", top: "50%", left: "50%", transform: "translate(-50%, -50%)    "}}>
                        ...
                        <input onChange={f} type="file" multiple style={{position: "absolute", opacity: 0, width: "100%", height: "100%", zIndex: 9, cursor: "pointer", top: 0, left: 0}} />
                    </div>
                }
                {/* rule */}
                <div className={"djkjaksjkajraeaw"} style={{position: "absolute", bottom: 0, textAlign: "center", fontSize: 12, fontWeight: 600, width :'100%'}}>Vui lòng đăng tải 5 hình ảnh</div>
            </div>
            <div style={{width: "100%", display: "flex", justifyContent: 'center', alignItems: "center", marginTop: 16}}>
                <button onClick={testUpload} style={{padding: "10px 30px", display: "flex", justifyContent: 'center', alignItems: 'center', cursor: "pointer", border: "none", outline: "none", background: "#2e89ff", color: "#fff", fontWeight: 600}}>Đăng tải</button>
            </div>  
        </div>
    )
}
